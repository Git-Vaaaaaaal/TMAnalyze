...
dataframe_id = os.path.join("csv_calym", "multi_label_patient_id.csv")
...
os.makedirs("output_calym/rfs_kfold", exist_ok=True)
log_path = "logs_rf_survival.txt"
results  = []

parameters_df = pd.read_csv("output_calym/rfs/results_rfs.csv")

COLORS = {"OS": "steelblue", "PFS": "tomato"}

for encoder in encoder_list:
    enc          = ENCODER_CFG[encoder]
    features_csv = os.path.join("data_224", encoder, enc["slide_subdir"], enc["slide_csv"])

    df_marker = parameters_df[parameters_df["encoder"] == encoder]
    ...
    for element_time in ["Overall survival (years)", "PFS (years)"]:
        ...
        with open(log_path, "a") as f:
            f.write(f"{element_time},{encoder},c_index_train={best_c_train:.4f},c_index_test={c_index_test:.4f}\n")
    ...
    if km_data:
        ...
        ax.legend()
        ax.grid(True)
        fig.savefig(f"output_calym/rfs_kfold/{encoder}_kfolds.png", dpi=150, bbox_inches="tight")
        plt.close(fig)

csv_path = "output_calym/rfs_kfold/results_kfold.csv"
