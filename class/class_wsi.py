...
class WSI:
    ...
    def extract_tissue_coords(
        ...
        -------
        pd.DataFrame
            DataFrame avec une ligne par patch et deux colonnes : 'x' et 'y'.

        Examples
        --------
        >>> df = wsi.extract_tissue_coords(20, 256, "output_coords", overlap=32)
        >>> print(df.head())
              x      y
        0  1024   2048
        1  1024   2304
    ...
    def extract_patch_features(
        ...
        coords_path : str
            Path to the CSV file containing patch coordinates (colonnes 'x' et 'y').
        save_features : str
        ...
        -------
        pd.DataFrame
            DataFrame avec une ligne par patch, colonnes : 'x', 'y', 'feat_0', ..., 'feat_D'.
            Également sauvegardé en CSV dans save_features/{wsi.name}.csv.

        Examples
        --------
        >>> df = wsi.extract_patch_features(patch_encoder, "patches/sample_patches.csv", "output_features")
        >>> print(df.head())
            x      y  feat_0  feat_1  ...
        0  1024   2048    0.23    0.11  ...
        """
        ...
        # ← Lecture du CSV de coordonnées produit par extract_tissue_coords
        ...
        # ← Construction du DataFrame x, y, feat_0, ..., feat_D
    ...
    def extract_slide_features(
        ...
        ----------
        coords : np.ndarray
            Tableau de shape (N, 2) contenant les coordonnées x, y de chaque patch,
            lu depuis le CSV produit par extract_patch_features.
        features : np.ndarray
            Tableau de shape (N, D) contenant les features de chaque patch,
            lu depuis le CSV produit par extract_patch_features.
        slide_encoder : torch.nn.Module
        ...
        -------
        pd.DataFrame
            DataFrame avec une seule ligne correspondant au WSI :
            colonnes 'wsi_name', 'slide_feat_0', ..., 'slide_feat_K'.
            Également sauvegardé en CSV dans save_features/{wsi.name}.csv.
        """
        ...
        # ← Conversion des arrays NumPy en tenseurs (déjà lus depuis le CSV en amont)
        patch_features = torch.from_numpy(features).float().to(device)
        ...
        # ← coords_attrs supprimé : plus de métadonnées H5, le CSV ne les porte pas
        batch = {
        ...
        slide_features = slide_features.float().cpu().numpy().squeeze()  # (K,)

        # ← Construction du DataFrame : une ligne = un WSI
        feat_cols = [f'slide_feat_{i}' for i in range(slide_features.shape[0])]
        row_df = pd.DataFrame(
            data=slide_features.reshape(1, -1),
            columns=feat_cols,
        )
        row_df.insert(0, 'wsi_name', self.name)
