...
for encoder in encoder_list:
    ...
    for marker in marker_list:
        candidate = os.path.join("input_kmeans", encoder, marker, enc["slide_subdir"], enc["slide_csv"])
        ...
        print(f"[{encoder}][{marker}] Silhouette (k=2) : {sil_avg:.4f}")

        df_clusters  = pd.DataFrame({"patient_id": df_feat["patient_id"], "cluster": labels})
        df_clinical  = pd.read_csv(dataframe_id)
        df_clin_mark = df_clinical[df_clinical["stain"] == marker]

        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        fig.suptitle(f"{encoder} | {marker}  —  KMeans k=2  (silhouette={sil_avg:.3f})")
        ...
        for ax_km, element_time in zip(axes[:2], ["OS", "PFS"]):
            ...
            cox_results.append({
                "encoder": encoder, "marker": marker, "time": element_time,
                "n": len(df_merged),
        ...
        plt.tight_layout()
        plt.savefig(os.path.join("kmeans", f"{marker}_{encoder}_cox_age.png"),
